...
//5、使用sqlite3_finalize和sqlite3_close函数释放资源
    public func creat(_ model: Events) -> Int {
        if self.openDB() {
    ...
    public func findAll() -> [Events] {
        ...
        if self.openDB() {
            let sql = "SELECT EventName, EventIcon, KeyInfo, BasicsInfo, OlympicInfo EventID FRONM Events"
            let cSql = sql.cString(using: String.Encoding.utf8)
            ...
            if sqlite3_prepare_v2(db, cSql, -1, &statement, nil) == SQLITE_OK {
                ...
                while sqlite3_step(statement) == SQLITE_ROW {
                    ...
                    if let strEeventIcon = getColumnValue(index: 1, stmt: statement!) {
                        events.EventIcon = strEeventIcon
                    }
